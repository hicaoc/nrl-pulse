...
pub fn decode_block(payload: &[u8], mut valprev: i32, mut index: i32) -> Vec<u8> {
    ...
    for &byte in payload {
        for nib in [byte >> 4, byte & 0x0F] {
            ...
            valprev = if nib & 8 != 0 { valprev - diff } else { valprev + diff };
...
pub fn resample_8k_to_48k(pcm: &[u8]) -> Vec<u8> {
    ...
    let samples: Vec<i32> = pcm.chunks_exact(2)
        .map(|c| i16::from_le_bytes([c[0], c[1]]) as i32).collect();
...
pub fn encode_block(pcm: &[u8], mut valprev: i32, mut index: i32)
    -> (Vec<u8>, i32, i32) {
    index = clamp(index, 0, 88);
    let n = pcm.len() / 2;
    ...
    for chunk in pcm.chunks_exact(2) {
        ...
        valprev = if nibble & 8 != 0 { valprev - vpdiff } else { valprev + vpdiff };
        valprev = clamp(valprev, -32768, 32767);
